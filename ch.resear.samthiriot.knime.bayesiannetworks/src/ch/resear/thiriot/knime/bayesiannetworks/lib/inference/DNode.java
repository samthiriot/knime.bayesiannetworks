...
public final class DNode {
	...
	private final LRUMap<Map<NodeCategorical,String>,Double> getCache() {
		...
		if (cacheEvidenceInContext2proba == null) {
			...
			if (logger.isInfoEnabled())
				logger.info("cache: max values to query: "+card+", will cache "+toCache+"\n"+this);
	...
	protected static DNode compose(CategoricalBayesianNetwork bn, Set<DNode> toCompose) {
		...
		if (logger.isDebugEnabled())
			logger.debug("composing " + toCompose);
		...
		if (logger.isDebugEnabled())
			logger.debug("composed "+toCompose+" into "+ res);
	...
	public Set<NodeCategorical> varsUnion() {
		...
		if (varsUnion == null) {
			...
			if (logger.isDebugEnabled())
				logger.debug("computed vars " + varsUnion);
		}
	...
	public Set<NodeCategorical> varsInter() {
		...
		if (varsInter == null) {
			...
			if (logger.isDebugEnabled())
				logger.debug("computed varsInter " + varsInter);
		}
	...
	public Set<NodeCategorical> cutset() {
		...
		if (cutset == null) {
			...
			this.cutset = cutset;
			if (logger.isDebugEnabled())
				logger.debug("computed cutset " + cutset);
		}
		return cutset;
	}
	...
	public Set<NodeCategorical> acutset() {
		...
		if (acutset == null) {
			...
			if (logger.isDebugEnabled())
				logger.debug("computed acutset " + acutset);
		}
	...
	protected double lookup(Map<NodeCategorical,String> n2v) {
		...
		if (logger.isDebugEnabled())
			logger.debug("Lookup on "+f+" for "+n2v);

		//String vMe = n2v.get(this.n);
			
		if (!n2v.containsKey(this.n)) {
			if (logger.isDebugEnabled())
				logger.debug("Not concerned by evidence => 1.");
		...
		*/
		...
		coord.keySet().retainAll(f.getVariables());
	...
	public double recursiveConditionning(Map<NodeCategorical,String> n2v) {
		...
		if (logger.isDebugEnabled()) 
			logger.debug("Recursive Conditionning for "+n2v+" on:\n "+this);
		
		if (isLeaf()) {
			if (logger.isDebugEnabled())
				logger.debug("is leaf => lookup");
			return lookup(n2v);
		}
		...
		if (!context().isEmpty() && !y.isEmpty()) {
			if (logger.isDebugEnabled())
				logger.debug("search in cache " + y);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("no leaf => summing over cutset " + cutset().stream().map(v->v.name).collect(Collectors.joining(",")));
		}
		double p = 0.;
		...
		if (logger.isDebugEnabled()) {
			logger.debug("no leaf => exploring combinations over " + uninstantiated.stream().map(v->v.name).collect(Collectors.joining(",")));
		}
		...
		for (IteratorCategoricalVariables it = bn.iterateDomains(uninstantiated); it.hasNext(); ) {
			...
			if (logger.isDebugEnabled())
				logger.debug("Sum of "+instantiation+" over\n " + this);

			
				
			double pLeft = first.recursiveConditionning(instantiation);
			if (logger.isDebugEnabled())
				logger.debug("Sum of "+instantiation+" = "+pLeft);
			if (pLeft == 0.)
			...
			if (logger.isDebugEnabled())
				logger.debug("Sum of "+instantiation+" = " + pRight);
		...
		*/
		...
		if (candidate.isLeaf()) {
			if (challenger.isLeaf()) {
				return candidate.f.getVariables().size() <= challenger.f.getVariables().size();
			} else {
				return true;
			}
		} else {
	...
	public void instanciate(Map<NodeCategorical, String> evidenceVariable2value) {
		...
		if (logger.isDebugEnabled())
			logger.debug("reducing "+this+" based on evidence " + evidenceVariable2value);
	...
	public void generate(Map<NodeCategorical, String> defined, AbstractContinousDistribution rng) {

		if (logger.isDebugEnabled())
			logger.debug("generating for "+this+" and known "+defined);
		...
		if (isLeaf()){
			...
			if (logger.isDebugEnabled())
				logger.debug("picking a value from our factor " + this.f);
			...
			for (Map.Entry<Map<NodeCategorical,String>,Double> e: reduced.values.entrySet()) {
				...
				if (cumulated >= random) {
					...
					if (logger.isDebugEnabled())
						logger.debug("picked from CPT: " + e.getKey());
					return; // stop all !
				} 
			}
			...
			if (logger.isDebugEnabled())
				logger.debug("calling left " + left);
			left.generate(defined, rng);
			if (logger.isDebugEnabled())
				logger.debug("calling right " + right);
			right.generate(defined, rng);	
		} 
